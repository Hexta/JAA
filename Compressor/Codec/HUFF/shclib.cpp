...
void
sh_CalcDecode(uchar *len, uchar *symb, uchar *base, uchar *offs, uchar *cache, int n) {
  for (int root = 0, j = 0, i = len[symb[0]]; i >= 0; --i) {
    int leaf = 0;
  ...
  for (int i = 0, j; i < SH_CACHESIZE; ++i) {
...
int
sh_EncodeBlock(uchar *iBlock, uchar *oBlock, unsigned int bSize) {
  int freq[SH_MAX_ALPHA];
  uchar symb[SH_MAX_ALPHA];
  uchar *len, *code, *aux;
  uint i, j, n, treeSize, streamSize, bits;
  uint bitbuf;
  uint *pBlock;
  ...
  *((int *) oBlock) = bSize;
  ...
  /* Encode a stream */
  for (i = j = 0; i < bSize; ++i)
    EncodeSymbol(len, code, iBlock[i], pBlock, j, bits, bitbuf);
...
int
sh_DecodeBlock(uchar *iBlock, uchar *oBlock, int bSize) {
  uchar len [SH_MAX_ALPHA];
  memset(len, 0, SH_MAX_ALPHA);
  uchar symb[SH_MAX_ALPHA];
  memset(symb, 0, SH_MAX_ALPHA);
  uchar base[SH_MAX_CLEN];
  uchar offs[SH_MAX_CLEN];
  uchar *cache;
  uint n, streamSize, treeSize, bits, symbol;
  uint bitbuf, *pBlock;
  ...
  /* Decode a stream */
  for (uint i = 0, j = 1; i < streamSize; ++i) {
    DecodeSymbol(base, offs, cache, symb, pBlock, j, bits, bitbuf, symbol);
...
void
sh_GetFreq(int *freq, uchar *block, int size) {
  memset(freq, 0, sizeof ( int) *SH_MAX_ALPHA);
...
void
sh_CalcLen(int *freq, uchar *symb, uchar *len, int n, int maxLen) {
...
sh_SortLen(uchar *len, uchar *symb, int n) {
  ...
  for (s = 0; s >= 0; --s) {
    ...
    while (l < h) {
      ...
      /* Partition loop */
      while (1) {
        while (i <= j && cmp(len[symb[i]], lm, symb[i], sm))
          ++i;
        while (cmp(lm, len[symb[j]], sm, symb[j]))
          --j;
        if (i >= j)
          break;
        swap(symb[i], symb[j]);
...
void
sh_CalcCode(uchar *len, uchar *symb, uchar *code, int n) {
  for (int prev = len[symb[0]], i = 0, codeval = 0; i < n; ++i, ++codeval) {
    int next = len[symb[i]];
    codeval >>= (prev - next);
    prev = next;
    code[symb[i]] = codeval;
...
int
sh_ExpandTree(uchar *len, uchar *symb, uint *buf) {
