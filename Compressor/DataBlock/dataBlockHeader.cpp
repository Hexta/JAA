...
void DataBlockHeader::setCodecParams(uint32_t codec_params) {
    d->headerData.codecParams = codec_params;
}

uint32_t DataBlockHeader::getCodecParams() {
    return d->headerData.codecParams;
}

void DataBlockHeader::setRAWDataSize(uint32_t RAWDataSize) {
    d->headerData.rawDataSize = RAWDataSize;
}

uint32_t DataBlockHeader::getRAWDataSize() const {
    return d->headerData.rawDataSize;
}

void DataBlockHeader::setEncodedDataSize(uint32_t encodedDataSize) {
    d->headerData.encodedDataSize = encodedDataSize;
}

uint32_t DataBlockHeader::getEncodedDataSize() const {
    return d->headerData.encodedDataSize;
}

void DataBlockHeader::setDecodedDataSize(uint32_t decodedDataSize) {
    d->headerData.decodedDataSize = decodedDataSize;
}

uint32_t DataBlockHeader::getDecodedDataSize() const {
    return d->headerData.decodedDataSize;
}

void DataBlockHeader::setId(uint64_t id) {
    d->headerData.id = id;
}

uint64_t DataBlockHeader::getId() const {
    return d->headerData.id;
}

unsigned char* DataBlockHeader::getData() {
    return reinterpret_cast<unsigned char*>(&d->headerData);
}

void DataBlockHeader::setData(unsigned char* inHeaderData) {
    memcpy(&d->headerData, inHeaderData, HEADER_SIZE);
}

void DataBlockHeader::clean() {
    memset(&d->headerData, 0, HEADER_SIZE);
}

void DataBlockHeader::setDataCRC(uint32_t crc) {
    d->headerData.encodedDataCRC = crc;
}

uint32_t DataBlockHeader::getDataCRC() {
    return d->headerData.encodedDataCRC;
}

void DataBlockHeader::recordCRC() {
    setHeaderCRC(calcCRC());
}

int DataBlockHeader::checkCRC() {
    if (calcCRC() != getHeaderCRC())
        return -1;
    return 0;
}

void DataBlockHeader::setFileName(const char* fileName) {
    strncpy(d->headerData.fileName, fileName, MAX_FILENAME_LENGTH);
}

char* DataBlockHeader::getFileName() {
    return d->headerData.fileName;
}

void DataBlockHeader::setPart(const uint32_t part) {
    d->headerData.part = part;
}

uint32_t DataBlockHeader::getPart() const {
    return d->headerData.part;
}

void DataBlockHeader::setPartsCount(const uint32_t partsCount) {
    d->headerData.partsCount = partsCount;
}

uint32_t DataBlockHeader::getPartsCount() const {
    return d->headerData.partsCount;
}

uint32_t DataBlockHeader::calcCRC() {
    getData();
...
void DataBlockHeader::setHeaderCRC(uint32_t headerCRC) {
    d->headerData.headerCRC = headerCRC;
}

uint32_t DataBlockHeader::getHeaderCRC() const {
    return d->headerData.headerCRC;
}

void DataBlockHeader::setOffset(uint64_t offset) {
    d->headerData.offset = offset;
}

uint64_t DataBlockHeader::getOffset() {
    return d->headerData.offset;
}

void DataBlockHeader::initRAW(uint64_t offset, const uint32_t rawDataSize) {
