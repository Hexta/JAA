...
Compressor::Compressor( CompressorStatus *_status ) : codec( ), block( NULL ), status( _status ), currReadBytesCount( 0 ), decodedDataSize( 0 ), encodedDataSize( 0 ), totalProcessingTime( 0 ),
...
Compressor::createEmptyFile( const char * fileName )
{
	...
	if ( !fout.open( QIODevice::Truncate | QIODevice::WriteOnly ) )
		return false;
	fout.close( );
	return true;
}
...
Compressor::compress( const QStringList &iFileNames, unsigned int iFilesCount, const QString &oFileName, unsigned int blocksize, const list< CoderTypes > *compressSequence )
{
	...
	for ( unsigned int i = 0; i < iFilesCount; ++i )
	{
	...
	bool stop = false;
	for ( vector<int>::iterator i = proceedFiles.begin( ); i != proceedFiles.end( ); ++i )
	{
		...
		while ( 1 )
		{
			...
			showEncodingProgress( iFileNames[*i], speed( source_data_size, stoptTime - startTime ) );
		}
		fin.close( );
		if ( !stop )
...
CompressorStatus::ErrorCode
Compressor::decompress( const QString &iFileName, bool keepBroken )
{
	blocksTable.clean( );
	totalProcessingTime = 0;
	decodedDataSize = 0;
	...
	while ( !stop )
	{
		switch ( block->read( fin ) )
		{
			...
			case FILE_BROKEN:
			{
				...
				id = id ? id : nextId++;
				showInfo( CompressorStatus::INPUT_FILE_CORRUPTED, header->getFileName( ), id );
				continue;
			}
		...
		showDecodingProgress( header->getFileName( ), speed( header->getEncodedDataSize( ), stoptTime - startTime ) );
		...
		switch ( blocksTable.add( header, nextId ) )
		{
			...
			case ALL_BLOCKS_RECIEVED:
				showInfo( CompressorStatus::SUCCESS, header->getFileName( ), blocksTable.getId( header ) );
				blocksTable.remove( header );
				break;
			case BLOCK_OUT_OF_RANGE:
			case BLOCK_ALREADY_RECIEVED: showInfo( CompressorStatus::INPUT_FILE_CORRUPTED );
				return error = CompressorStatus::INPUT_FILE_CORRUPTED;
				break;
			default:break;
		}
		...
		{
			stop = true;
			showInfo( CompressorStatus::CANCELLED, header->getFileName( ), blocksTable.getId( header ) );
		}
	}
	...
	if ( brokenFilesNames )
		for ( vector<string>::iterator i = brokenFilesNames->begin( ); i != brokenFilesNames->end( ); ++i )
		{
			showInfo( CompressorStatus::INPUT_FILE_CORRUPTED, i->c_str( ), blocksTable.getId( *i ) );
			if ( keepBroken == false )
				removeBrokenFiles( );
		}
...
{
	vector<string> *nonCompleteFilesNames = blocksTable.getNonCompleteFilesNames( );

	if ( nonCompleteFilesNames )
		stat->brokenFilesNames = *nonCompleteFilesNames;
...
Compressor::removeBrokenFiles( )
{
	...
	if ( nonCompleteFilesNames )
	{
		for ( unsigned int i = 0; i < nonCompleteFilesNames->size( ); ++i )
			QFile::remove( QString::fromUtf8( ( *nonCompleteFilesNames )[i].c_str( ), strlen( ( *nonCompleteFilesNames )[i].c_str( ) ) ) );
	}
}

CompressorStatus::ErrorCode
Compressor::listArchiveContents( const QString &iFileName, vector<FilesTable::FileInfo> * const contents )
{
	blocksTable.clean( );
	unsigned int nextId = 1;
	...
	while ( !stop )
	{
		...
		switch ( blocksTable.add( header, nextId ) )
		{
			case FIRST_AND_LAST_RECIEVED_BLOCK:
				blocksTable.remove( header );
				showInfo( CompressorStatus::INPUT_FILE_UNCORRUPTED, header->getFileName( ), nextId++ );
				break;
			case FIRST_RECIEVED_BLOCK:
				showInfo( CompressorStatus::PROCEED, header->getFileName( ), nextId++ );
				break;
			case ALL_BLOCKS_RECIEVED:
				showInfo( CompressorStatus::INPUT_FILE_UNCORRUPTED, header->getFileName( ), blocksTable.getId( header ) );
				blocksTable.remove( header );
				break;
			case BLOCK_OUT_OF_RANGE:
			case BLOCK_ALREADY_RECIEVED: error = CompressorStatus::INPUT_FILE_CORRUPTED;
				showInfo( error );
				delete block;
				return error;
				break;
	...
	delete block;
		...
		if ( contents )
		{
			vector<FilesTable::FileInfo> *partialArchiveContents = blocksTable.getArchiveContent( );
			contents->insert( contents->end( ), partialArchiveContents->begin( ), partialArchiveContents->end( ) );
		}
	return error;
}
