...
public:
    ...
    CompressorStatus( ) : runStatus( RUN ) { };
    ...
    enum ErrorCode
    {
        SUCCESS, INPUT_FILE_OPEN_ERROR, OUTPUT_FILE_WRITE_ERROR, INPUT_FILE_CORRUPTED, INPUT_FILE_UNCORRUPTED, DECOMPRESS_FAIL, PROCEED, CANCELLED
    } ;
...
public:
     ...
     * @return				error code
     */
    CompressorStatus::ErrorCode		compress( const QStringList  &iFileNames, unsigned int iFilesCount, const QString &oFileName,
                                                unsigned int blocksize, const list< CoderTypes > *compressSequence = NULL );
    /**
     * Decompress archive
     ...
     * @return			error code
     */
    CompressorStatus::ErrorCode		listArchiveContents( const QString &iFileName, vector<FilesTable::FileInfo> * const contents = NULL );
...
private:
    ...
    void							showInfo( CompressorStatus::ErrorCode errorCode, const QString &currFileName = "", unsigned int id = 0 );
...
public:
    ...
    void operator( )( Compressor::CoderTypes coderType )
    {
        switch ( coderType )
        {
            case Compressor::BWT: codec->encode_BWT( block );
                break;
            case Compressor::RLE: codec->encode_RLE( block );
                break;
            case Compressor::MTF: codec->encode_MTF( block );
                break;
            case Compressor::HUFFMAN: codec->encode_HUFF( block );
                break;
            default:
                break;
        }
...
void inline		Compressor::compress( const list< CoderTypes > *compressSequence )
{
    ...
    const list< CoderTypes > *sequence = compressSequence ? compressSequence : &defaultCompressSequence;
...
bool inline		Compressor::decompress( )
{
    ...
    while ( ( id = block->getHeader( )->getId( ) ) != RAW_ID )
    {
        switch ( id )
        {
            case HUFF_ID: codec.decode_HUFF( block );
                break;
            case RLE_ID: codec.decode_RLE( block );
                break;
            case BWT_ID: codec.decode_BWT( block );
                break;
            case MTF_ID: codec.decode_MTF( block );
                break;
            default:return - 1;
        }
    }
...
{
    if (status)
        status->showProgress( ( encodedDataSize ) ? ( float ) currReadBytesCount / encodedDataSize : 0, QString::fromUtf8( currFileName, strlen( currFileName ) ), speed );
}

void inline			Compressor::showEncodingProgress( const QString &currFileName, const float speed )
{
    if (status)
        status->showProgress( ( decodedDataSize ) ? ( float ) currReadBytesCount / decodedDataSize : 0, currFileName, speed );
}
...
void inline			Compressor::showInfo( CompressorStatus::ErrorCode errorCode, const QString &currFileName , unsigned int id )
{
    if (status)
        status->showInfo( errorCode, currFileName, id );
}

void inline			Compressor::showInfo( CompressorStatus::ErrorCode errorCode, const char * currFileName , unsigned int id )
{
    QString fileName;
    if (! currFileName)
        fileName = "";
    ...
    if (status)
        status->showInfo( errorCode, fileName, id );
}
