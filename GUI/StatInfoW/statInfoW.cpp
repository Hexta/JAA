...
StatInfoW::showInfo( StatInfo info )
{
	...
	if ( !compressSequence.empty( ) )//Compress Mode
	{
		ratioLabel->setText( QString::number( ( float ) 100 * info.outputSize / info.inputSize, 'f', 2 ) + " %" );
		for ( QList< Compressor::CoderTypes>::iterator i = compressSequence.begin( ); i != compressSequence.end( ); ++i )
		{
			switch ( *i )
			{
				case Compressor::BWT: encodingSequenceLabel->setText( encodingSequenceLabel->text( ) + "BWT " );
					break;
				case Compressor::HUFFMAN:encodingSequenceLabel->setText( encodingSequenceLabel->text( ) + "HUFFMAN " );
					break;
				case Compressor::MTF:encodingSequenceLabel->setText( encodingSequenceLabel->text( ) + "MTF " );
					break;
				case Compressor::RLE:encodingSequenceLabel->setText( encodingSequenceLabel->text( ) + "RLE " );
					break;
